...
class AITrackActor(BaseActor):
    ...
    def __init__(self, net, objective, loss_weight, settings, bins, search_size, cfg=None):
        ...
        self.tem_mask = {}
        #self.sea_mask = {}
        self.pred_bbox = None   # Used to generate search mask [x_min, y_min, x_max, y_max]
    ...
    def get_subwindow(self, im, pos, model_sz, original_sz, avg_chans, im_mask=None):
        """
        args:
            im: bgr based image
            pos: center position
        ...
        if any([top_pad, bottom_pad, left_pad, right_pad]):
            ...
            if im_mask is not None:
                size_mask = (r + top_pad + bottom_pad, c + left_pad + right_pad)
                te_im_mask = np.zeros(size_mask, np.uint8)
                te_im_mask[top_pad:top_pad + r, left_pad:left_pad + c] = im_mask
            ...
            if im_mask is not None:
                im_patch_mask = te_im_mask[int(context_ymin):int(context_ymax + 1),
                                        int(context_xmin):int(context_xmax + 1)]
        else:
            im_patch = im[int(context_ymin):int(context_ymax + 1),
                       int(context_xmin):int(context_xmax + 1), :]
            if im_mask is not None:
                im_patch_mask = im_mask[int(context_ymin):int(context_ymax + 1),
                                        int(context_xmin):int(context_xmax + 1)]

        #masked_img = cv2.bitwise_and(im_patch,im_patch,mask=im_patch_mask)
        ...
        if not np.array_equal(model_sz, original_sz):
            try:
                ...
                if im_mask is not None:
                    im_patch_mask = cv2.resize(im_patch_mask, (model_sz, model_sz), 
        ...
        if im_mask is None:
            ...
            im_patch_mask = im_patch_mask[np.newaxis, :, :]
            im_patch_mask = im_patch_mask.astype(np.float32)
            im_patch_mask = torch.from_numpy(im_patch_mask)
            im_patch_mask = im_patch_mask.cuda()
            
            return im_patch, im_patch_mask
    ...
    def batch_init(self, images, template_bbox, initial_bbox) -> dict:
        ...
        gt_bbox_mask = bbutils.batch_center2corner(template_bbox)
        
        for i in range(len(images)):
            # Compute binary mask from image shape and bounding box
            W, H, _ = images[i].shape
            mask = np.zeros((W, H), dtype=np.uint8)
            bb = [int(x) for x in gt_bbox_mask[i]]
            mask[bb[1]:bb[3], bb[0]:bb[2]] = 255
            
            
            #masked_img = cv2.bitwise_and(images[i],images[i],mask=mask)
            #cv2.imshow('Masked', cv2.resize(masked_img.transpose((1,0,2)), (940, 540)))
            
            
            here_crop, here_crop_mask = self.get_subwindow(images[i], template_bbox[i, :2],
                                           self.cfg.DATA.TEMPLATE.SIZE, s_z[i], 
                                           self.channel_average[i], im_mask=mask)
            z_crop = here_crop.float().mul(1.0 / 255.0).clamp(0.0, 1.0)
            z_crop_mask = here_crop_mask.float().mul(1.0 / 255.0).clamp(0.0, 1.0)
            self.mean = [0.485, 0.456, 0.406]
            self.std = [0.229, 0.224, 0.225]
            self.inplace = False
            z_crop[0] = tvisf.normalize(z_crop[0], self.mean, self.std, self.inplace)
            z_crop_list.append(z_crop.clone())
            z_crop_mask_list.append(z_crop_mask.clone())
            
        z_crop = torch.cat(z_crop_list, dim=0)  # Tensor(2*num_seq,3,128,128)
        z_crop_mask = torch.cat(z_crop_mask_list, dim=0)

        self.update_rem = None
        self.tem_mask = z_crop_mask
        self.pred_bbox = gt_bbox_mask

        out = {'template_images': z_crop,
               'template_masks': z_crop_mask}
        return out
    ...
    def batch_track(self, img, gt_boxes, template, action_mode='max') -> dict:
        ...
        for i in range(len(img)):
            ...
            W, H, _ = img[i].shape
            mask = np.zeros((W, H), dtype=np.uint8)
            bb = [int(x) for x in self.pred_bbox[i]]
            mask_w, mask_h = bb[2]-bb[0], bb[3]-bb[1]
            pp = [int(x) for x in self.center_pos[i]]
            bb[0], bb[2] = int(pp[0] - mask_w/2), int(pp[0] + mask_w/2)
            bb[1], bb[3] = int(pp[1] - mask_h/2), int(pp[1] + mask_h/2)
            mask[bb[1]:bb[3], bb[0]:bb[2]] = 255
            
            channel_avg = np.mean(img[i], axis=(0, 1))
            x_crop, x_crop_mask = self.get_subwindow(img[i], self.center_pos[i], 
                                            self.cfg.DATA.SEARCH.SIZE,
                                            round(s_x[i]), channel_avg, mask)
            ...
            x_crop_mask = x_crop_mask.float().mul(1.0 / 255.0).clamp(0.0, 1.0)
            x_crop_mask_list.append(x_crop_mask.clone())

        x_crop = torch.cat(x_crop_list, dim=0)
        x_crop_mask = torch.cat(x_crop_mask_list, dim=0)
        pre_seq_output = torch.cat(pre_seq_in_list, dim=0).reshape(-1, 4 * self.pre_num)
        
        outputs = self.net(template, self.tem_mask, 
                           x_crop, x_crop_mask,
                           seq_input=pre_seq_output, 
                           head_type=None, stage="batch_track",
        ...
        # Update self.pred_bbox
        self.pred_bbox = bbox

        out = {
            'search_images': x_crop,
            'search_masks': x_crop_mask,
            'pred_bboxes': bbox,
    ...
    def explore(self, data):
        ...
        for idx in range(np.max(num_frames)):
            ...
            if idx == 0:
                ...
                results['template_masks'] = outputs_template['template_masks']

            else:
                outputs = self.batch_track(here_images, here_gt_bbox, 
                                           outputs_template['template_images'],
                                           action_mode='half')
                if outputs == None:
                    return None
                ...
                search_masks_list.append(outputs['search_masks'])
                search_anno_list.append(outputs['gt_in_crop'])
                #if len(outputs['pre_seq']) != 8:
                if len(outputs['pre_seq']) != num_seq:
        ...
        results['search_images'] = torch.cat([torch.stack(search_images_list)], dim=1)
        results['search_masks'] = torch.cat([torch.stack(search_masks_list)], dim=1)
        results['search_anno'] = torch.cat([torch.stack(search_anno_list)], dim=1)
    ...
    def compute_sequence_losses(self, data):
        ...
        template_masks = data['template_masks'].repeat(num_frames, 1, 1, 1)
        template_masks = template_masks.view(-1, *template_masks.size()[2:])
        
        search_images = data['search_images'].reshape(-1, *data['search_images'].size()[2:])
        search_masks = data['search_masks'].reshape(-1, *data['search_masks'].size()[2:])
        ...
        outputs = self.net(template_images, template_masks,
                           search_images, search_masks,
                           seq_input=pre_seq, stage="forward_pass",
