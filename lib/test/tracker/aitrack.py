...
from lib.models.aitrack import build_aitrack
...
# Segmentation module
from seg_module.sta_helper import STAHelper
from seg_module.params import parameters


class AITrack(BaseTracker):
    def __init__(self, params, dataset_name):
        super(AITrack, self).__init__(params)
        network = build_aitrack(params.cfg, training=False)
        ...
        # Masks and Clip variables
        #self.template_clip = {}
        self.z_mask = {}
        self.x_mask = {}
        
        # Load segmentation Module
        self.seg_params = parameters('./seg_module/sta.pth.tar')
        self.seg_params.device = 'cuda'
        self.seg_predictor = STAHelper(self.seg_params)
    ...
    def initialize(self, image, info: dict):
        ...
        H, W, _ = image.shape
        bb = info['init_bbox']
        #x, y, w, h = list(bb) if not isinstance(bb, list) else bb
        #mask = np.zeros((H, W), dtype=np.int)
        #mask[int(y):int(y+h), int(x):int(x+w)] = 1
        
        # Compute segmentation mask
        mask = self.seg_predictor.predict_mask(image, np.array(bb, dtype='float32'))
        mask = np.array(mask.numpy(), dtype=np.int64)

        z_patch_arr, resize_factor, z_amask_arr, mask_crop = sample_target(image, info['init_bbox'], 
                                                                self.params.template_factor,
                                                                output_sz=self.params.template_size,
                                                                mask=torch.tensor(mask, dtype=torch.int8))  # output_sz=self.params.template_size
        self.z_mask = mask_crop.numpy()
        
        # input template Image for CLIP
    ...
    def track(self, image, info: dict = None):
        ...
        bb = [int(x) for x in self.state]
        
        # Compute segmentation mask
        mask = self.seg_predictor.predict_mask(image, np.array(bb, dtype='float32'))
        mask = np.array(mask.numpy(), dtype=np.int64)
        
        x_patch_arr, resize_factor, x_amask_arr, mask_crop = sample_target(image, self.state, 
                                                                           self.params.search_factor,
                                                                output_sz=self.params.search_size,
                                                                mask=torch.tensor(mask, dtype=torch.int8))  # (x1, y1, w, h)
        self.x_mask = mask_crop.numpy()
        ...
        with torch.no_grad():
            ...
            out_dict = self.network.forward(
                ...
                template_mask=self.z_mask,
                search=x_dict.tensors,
                search_mask=self.x_mask,
                seq_input=seqs_out, stage="sequence", 
                search_feature=self.x_feat, update=None)
        
        self.x_feat = out_dict['x_feat']

        pred_boxes = out_dict['seqs'][:, 0:4]
        pred_boxes = pred_boxes / (self.bins - 1) 
        pred_boxes = pred_boxes - 0.5
